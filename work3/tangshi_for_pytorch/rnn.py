...
class RNN_model(nn.Module):
    def __init__(self, batch_sz ,vocab_len ,word_embedding,embedding_dim, lstm_hidden_dim):
        ...
        # 定义双层LSTM，输入维度为embedding_dim，隐藏层维度为lstm_hidden_dim，batch_first=True
    ...
    def init_hidden(self, batch_size):
        # 初始化隐藏状态和细胞状态为零（双层LSTM）
        return (torch.zeros(2, batch_size, self.lstm_dim).to(device),
                torch.zeros(2, batch_size, self.lstm_dim).to(device))
    def forward(self,sentence,is_test = False):
        ...
        # 将输入传入LSTM
        output, _ = self.rnn_lstm(batch_input,hidden_cell)
